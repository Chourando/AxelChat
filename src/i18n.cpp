...
bool I18n::setLanguage(const QString &shortTag)
{
    ...
    if (normTag != _languageTag)
    {
        ...
        QString fileName = findTranslationFile(normTag);
        if (!fileName.isEmpty())
        {
        ...
        {
            qDebug(QString("No translation file found for tag \"%1\"")
                   .arg(shortTag).toUtf8());
            QLocale::setDefault(QLocale("C"));
            emit languageChanged();
            return false;
...
QString I18n::findTranslationFile(const QString &tag)
{
    QString normTag = tag.toLower().trimmed();
    if (normTag == "ru")
    {
        return ":/ru_RU.qm";
    }
